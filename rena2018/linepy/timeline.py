...
class LineTimeline(object):
    ...
    def __init__(self):
        if self.isLogin == True and self.channelId == self.server.CHANNEL_ID['LINE_TIMELINE']:
            self.client.log('LineTimeline attached')
    ...
    def getHome(self,mid):
        ...
        r = requests.get(
            "http://gd2.line.naver.jp/mh/api/v27/post/list.json?homeId=" + mid + "&commentLimit=2&sourceType=LINE_PROFILE_COVER&likeLimit=6", headers = header
        )
        return r.json()
